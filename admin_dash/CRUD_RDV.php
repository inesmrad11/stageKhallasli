...
<head>
    ...
    <title>Gestion des Rendez-vous - Khallasli CRM</title>
            ...
            <div class="content">
                <div class="container">
                    <div class="page-title">
                        <h3>Rendez-vous
                            <a href="add_RDV.php" class="btn btn-sm btn-outline-secondary float-end"><i class="fas fa-plus"></i> Ajouter Rendez-vous</a>
                        </h3>
                    </div>
                        ...
                        <div class="box-body">
                            <table width="100%" class="table table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                    <?php
                                    try {
                                        $stmt = $pdo->query("SELECT rendez_vous.*, prospects.name AS prospect_name, points_de_vente.name AS pdv_name
                                                             FROM rendez_vous
                                                             LEFT JOIN prospects ON rendez_vous.prospect_id = prospects.id
                                                             LEFT JOIN points_de_vente ON rendez_vous.point_de_vente_id = points_de_vente.id");
                                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                            echo "<tr>
                                                    ...
                                                    <td>{$row['notes']}</td>
                                                    <td class='text-end'>
                                                        <a href='edit_RDV.php?id={$row['id']}' class='btn btn-outline-primary btn-rounded'><i class='fas fa-pen'></i></a>
                                                        <a href='delete_RDV.php?id={$row['id']}' class='btn btn-outline-danger btn-rounded'><i class='fas fa-trash'></i></a>
                                                    </td>
                                                </tr>";
                                        }
                                    } catch (PDOException $e) {
